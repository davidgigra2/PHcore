...
import { updateUnitOwner } from "./units/actions";
import { revokeProxyForUnit } from "./units/proxy-actions";
...
export default function UnitsTab({ assemblyId, units: initialUnits, totalUnits, totalCoefficient, proxyMap }: UnitsTabProps) {
    ...
    // Sync local state whenever the server re-sends fresh props (e.g. after router.refresh())
    useEffect(() => {
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-violet-500/10 border border-violet-500/20 rounded-lg">
                    ...
                    <span className="text-xs text-violet-300 font-medium">Coef. Total: {totalCoefficient.toFixed(4)}</span>
                </div>
                <div className="ml-auto flex items-center gap-2">
                    <Button
                        ...
                        className="bg-white/5 border-white/10 text-gray-400 hover:bg-white/10 hover:text-white h-8 w-8 p-0 disabled:opacity-50"
                    >
                        <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? 'animate-spin' : ''}`} />
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setShowUpload(v => !v)}
                        className="bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:text-white text-xs h-8 gap-1.5">
            ...
            {showUpload && (
                <div className="bg-[#111] border border-white/5 rounded-xl p-5 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h2 className="text-white font-semibold text-sm">Carga Masiva de Unidades</h2>
                    </div>
                            ...
                            <tbody>
                                {filtered.map((u: any, i: number) => (
                                    <tr key={u.id} className={`border-t border-white/5 hover:bg-white/[0.035] transition-colors ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>
                                        ...
                                        <td className="px-4 py-3">
                                            {u.representative ? (() => {
                                                const ownerDoc = String(u.owner_document_number || '').trim().toLowerCase();
                                                const repDoc = String(u.representative.document_number || '').trim().toLowerCase();
                                                const isSelf = ownerDoc && ownerDoc === repDoc;
                                                const proxy = proxyMap[u.representative.id] || null;
                                                    ...
                                                    <div className="flex items-start gap-2">
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            {u.representative.document_number && (
                                                                <div className="flex items-center gap-1 text-gray-500 text-[10px] mt-0.5">
                                                                    <CreditCard className="w-2.5 h-2.5" />{u.representative.document_number}
                                                                </div>
                                                            )}
                                                            {proxy && (
            ...
            {/* Edit dialog */}
            {editingUnit && (
                <EditOwnerDialog unit={editingUnit} onClose={() => setEditingUnit(null)} onSaved={handleSaved} />
            )}
        </div>
    );
}
